...
int main(int argc, char **argv) {
    ...
    MPI_Gather(&local_sum, 1, MPI_LONG_LONG, all_sums, 1, MPI_LONG_LONG,
               0, MPI_COMM_WORLD);
    if (rank == 0) {
        long long total_sum = 0;
        for (int i = 0; i < size; i++) total_sum += all_sums[i];
        long long expected = (long long)N * (N + 1) / 2;
        printf("[Gather] Total = %lld, Expected = %lld, Correct? %s, Time = %.4f sec\n",
               total_sum, expected, total_sum == expected ? "YES" : "NO",
               MPI_Wtime() - start);
    }
